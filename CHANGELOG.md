...
- `git-pr`: PR 본문을 마크다운 문서로 먼저 확인하는 미리보기 모드 추가 (`--preview` 또는 "미리보기 문서로 먼저 보여줘" 류 요청). 레포 루트에 `.pr-preview.md`를 만들어 에디터 마크다운 프리뷰로 머메이드 렌더링까지 확인한 뒤 생성하며, PR이 실제로 만들어진 뒤에만 미리보기 파일을 삭제 (생성 실패 시엔 남겨서 재시도 가능)
...
- `git-pr`: mermaid 다이어그램 기본값을 "그리지 않음"으로 전환. 다이어그램이 이해를 돕기는커녕 방해하던 문제를 R2 게이트 재작성으로 해결
  - 기존 OR 조건 중 `참여자 3개 이상`이 사실상 모든 PR을 통과시키던 문제 제거 — 폼 → 훅 → api → 캐시는 참여자 4개지만 여전히 직선 흐름이라는 반례를 명시
  - **선형 흐름은 즉시 탈락** — 직선 경로는 불릿이 이미 전달하는 내용인데 그림이 시선을 먼저 가져가서 정보량이 더 적은 쪽을 읽게 만듦
  - 통과 조건을 AND 4개로 변경. 그중 "이 다이어그램이 답하는 질문을 한 문장으로 쓸 수 있는가"가 핵심이며, 초안 완성 후 그 질문을 다이어그램만 보고 답해보는 자기검증 단계 추가 — 답이 안 나오거나 불릿만으로 답이 나오면 삭제
  - 상한을 그룹당 1개 → PR 전체 1개로 축소
- `git-pr`: 다이어그램은 전체 구조가 아니라 변경분만 그리도록 규정 — 이 PR이 추가/수정한 노드·엣지만 넣고, 문맥상 필요한 기존 노드는 `기존` 라벨로 표시. `기존` 표시가 절반을 넘으면 변경 설명이 아니므로 삭제
- `git-pr`: mermaid 렌더 안전 규칙 추가 — 파싱이 깨지면 raw 코드블록으로 노출돼 없느니만 못하므로, 특수문자 포함 라벨은 큰따옴표로 감싸고 노드 ID는 ASCII로 제한(한글은 라벨에만). 렌더러마다 가장 많이 갈리는 `%%{init}%%`·`classDef`·`style`·`<br/>`는 금지. `mermaid-forms.md`의 기존 예시도 이 규칙에 맞게 수정
- `git-pr`: `mermaid-forms.md`의 첫 번째 형태였던 "레이어 간 데이터 흐름"이 예시부터 직선이라 새 게이트와 모순되어 fan-in/fan-out 형태로 재작성
- `git-pr`: PR 본문 전달을 `--body "{BODY}"`에서 `--body-file`로 교체 — 본문에 머메이드 펜스(백틱)나 따옴표가 들어가면 셸 인터폴레이션에서 깨지던 문제
