...
- `implement`: Phase마다 접근법을 요약하고 승인을 받던 게이트 제거 — 계획은 `task-plan`에서 이미 합의된 것으로 보고 Phase를 안내만 한 뒤 바로 구현하고, 끝나면 다음 Phase로 자동 진행. 중단 지점을 예전의 "Phase 시작 전 매번"에서 실제로 사람이 필요한 순간으로 옮긴 것
  - 구현 사이클을 `구현 → 검증 → 기록`으로 변경. 검증은 프로젝트에 실제로 있는 명령(타입체크·린트·테스트·빌드)을 변경 범위에 맞춰 실행하고, 실패는 Phase 안에서 고쳐서 재실행. 같은 이유로 두 번 실패하면 계획이 틀렸다는 신호로 보고 중단
  - 중단 조건표 신설: 문서가 정하지 않은 결정 / PRD에 없는 사용자 노출 문구 / 공유 컴포넌트 수정·새 라이브러리 도입 / 검증 2회 실패 / 되돌리기 어려운 작업 / 사용자만 할 수 있는 일. 그 외(파일 배치, 네이밍, 헬퍼 분리, Phase 내부 순서, 자기 실수 수정)는 묻지 않고 진행
  - 중단 시 답에 의존하지 않는 작업은 뒤 Phase까지 먼저 끝낸 뒤 한 번에 모아서 질문
  - 중단된 작업 재개 시 "이어서 할까요?" 확인 제거 — 어디서부터 이어가는지만 한 줄로 알리고 계속. 단 `## 확인 필요`에 미답변 항목이 있으면 코드 작성 전에 먼저 질문
  - `## 진행 기록`에 `검증:` 줄 추가 — 실제로 실행한 명령만 기록 (`/git-pr`이 이 줄을 PR 테스트 케이스로 전사하므로 실행하지 않은 검증은 PR의 허위 기재가 됨)
- `implement`: `## 진행 기록`을 "무엇을 구현했는지" 기록에서 "체크리스트가 못 담는 것"만 남기는 기록으로 변경 — 체크박스에 이미 있는 구현 내역·파일 목록을 다시 쓰지 않고, 구현 중 내린 결정과 이유·계획과 달라진 점·블로커만 한 줄씩. Phase 완료 기본형은 `- Phase N 완료` 한 줄
- `implement`: 전체 완료 시 진행 기록에 요약을 다시 쓰지 않고 `- 전체 구현 완료` 한 줄만 추가

### Added

- `task-plan`: `## 확인 필요` 섹션 신설 — 계획만으로 결정 못 하는 항목을 한 줄 질문으로 모아두고, **답을 받으면 질문 줄을 삭제하고 결정된 내용만 남김**. 항목이 다 없어지면 섹션 자체를 삭제 (`tasks.template.md`에 자리 추가)
- `implement`: 중단할 때 **먼저 `spec.md`에 기록하고 그다음 대화로 질문**하는 2단계로 변경. `## 결정이 필요한 부분` 섹션에 `지금 상태 / 왜 막히는지 / 선택지 / 추천`을 사람이 읽을 수 있는 형태로 적는다 — 대화는 스크롤에 묻히고 다음 세션이 못 읽지만 설계서는 남기 때문. 선택지는 기술이 아니라 사용자가 보게 될 결과로 비교하고(`응답을 캐시한다` ✗ → `재진입이 즉시 뜨지만 다른 기기 변경이 1분 늦게 보인다` ✓), 답을 받으면 블록을 지우고 결정된 내용을 흐름·상태·엣지 케이스 본문에 녹임
  - 계획 단계 질문(`tasks.md`의 `## 확인 필요`)과 구현 중 막힌 결정(`spec.md`의 `## 결정이 필요한 부분`)을 구분 — 같은 항목을 양쪽에 쓰지 않음
  - 재개 시와 전체 완료 시 두 섹션의 미답변 항목을 확인 — 열린 질문이 남아 있으면 완료 처리하지 않음
- `implement`: 중단 시 쓰는 고정 양식(`### 확인이 필요합니다`) 추가 — 막힌 지점 / 필요한 것 / 선택지 / 추천 / 답을 받으면 할 일. 선택지와 추천을 같은 메시지에 넣고, 사용자 노출 문구는 지어내지 않고 반드시 질문

## [0.8.0] - 2026-08-05

### Added

- `git-pr`: PR 본문을 마크다운 문서로 먼저 확인하는 미리보기 모드 추가 (`--preview` 또는 "미리보기 문서로 먼저 보여줘" 류 요청). 레포 루트에 `.pr-preview.md`를 만들어 에디터 마크다운 프리뷰로 확인한 뒤 생성하며, PR이 실제로 만들어진 뒤에만 미리보기 파일을 삭제 (생성 실패 시엔 남겨서 재시도 가능)
  - 미리보기 파일을 사용자가 직접 수정해도 됨 — `수정할게요` 선택 시 파일을 다시 읽는 것부터 시작해 사용자의 편집 내용을 기준으로 삼음
  - 프리앰블(제목/head/base)과 본문 경계는 `<!-- ↓↓↓ PR 본문 시작 ↓↓↓ -->` 마커로 구분 — 본문 안에 `---`가 들어가므로 구분선을 경계로 쓰지 않음

### Removed

- `git-pr`: PR 본문의 mermaid 다이어그램 기능 전면 제거. 다이어그램이 이해를 돕기보다 방해하는 경우가 잦았음 — 그림이 불릿보다 시선을 먼저 가져가는데, 정작 담긴 정보는 불릿보다 적어서 리뷰어가 정보량이 더 적은 쪽을 먼저 읽게 됨. "언제 그릴지"를 판정하는 규칙을 다듬는 방향도 검토했으나, 판정 기준을 아무리 좁혀도 경계 사례에서 득실이 분명치 않아 기능 자체를 걷어내는 쪽을 택함
  - R2를 "다이어그램을 그릴지 판단하는 게이트"에서 "산문과 불릿만 사용한다"는 금지 규칙으로 교체. 흐름이 불릿으로 설명이 안 될 만큼 얽혀 있으면 그림을 그릴 게 아니라 그룹을 쪼개라는 지침으로 대체
  - `templates/mermaid-forms.md` 삭제 (다이어그램 형태 5종 + 공통 제약)
  - `pr-body.template.md`의 mermaid 블록 자리, `pr-body.example.md`의 다이어그램 관련 안내 문구 삭제

### Changed

- `git-pr`: PR 본문 전달을 `--body "{BODY}"`에서 `--body-file`로 교체 — 본문에 백틱·따옴표·`$`가 들어가면 셸 인터폴레이션에서 깨지던 문제
- `git-pr`: frontmatter `allowed-tools`에 `Read, Write, Edit` 추가 — 기존에도 템플릿 읽기와 충돌 해결 편집을 지시하면서 선언이 빠져 있었음
