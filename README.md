...
구현 사이클: Phase 안내 → 직접 구현 → 검증 → `tasks.md` 체크박스 토글 + `## 진행 기록` 한 줄 — 다음 Phase로 자동 진행

- 계획은 task-plan 단계에서 이미 합의된 것으로 보고, Phase마다 승인을 다시 묻지 않습니다.
- 멈추는 경우는 정해져 있습니다: 문서가 정하지 않은 결정, PRD에 없는 사용자 노출 문구, 공유 컴포넌트 수정이나 새 라이브러리 도입, 같은 이유로 검증이 두 번 실패, 되돌리기 어려운 작업, 사용자만 할 수 있는 일(시크릿·로그인·권한).
- 멈출 때는 `spec.md`의 `## 결정이 필요한 부분`에 `지금 상태 / 왜 막히는지 / 선택지 / 추천`을 먼저 기록하고, 대화에는 `막힌 지점 / 필요한 것 / 선택지 / 추천 / 답 받으면 할 일` 요약과 문서 위치를 남깁니다. 답을 받으면 그 블록은 지우고 결정된 내용만 명세 본문에 반영됩니다.
- 답에 의존하지 않는 작업은 먼저 끝내고, 막힌 것들을 한 번에 모아서 묻습니다.
