...
**테스트·QA·검증은 browse로만 한다.** 구현 결과 확인, 회귀 검증, 버그 재현, 반응형 점검, `/qa` 실행 — 이 범주에 들어가면 호스트에 다른 브라우저 도구가 있어도 browse를 쓴다.

이유는 취향이 아니라 결과물의 성질입니다. browse는 스냅샷 기준점을 서버가 들고 있어서 `snapshot -D`로 액션 전후 unified diff를 낼 수 있고, `@e`/`@c` ref가 트리 순서로 고정되어 재현 단계를 그대로 문서에 적을 수 있습니다. 도구를 섞는 순간 쿠키·세션·기준 스냅샷이 갈라져서 diff와 재현 단계가 전부 신뢰를 잃습니다.

| 상황 | 도구 |
|------|------|
| `/qa`, 구현 검증, 회귀 테스트, 버그 재현 | **browse 필수** |
| 액션 전후 비교, 반응형 3종, 폼 플로우, 콘솔·네트워크 증거 수집 | **browse 필수** |
| 공개 문서·레퍼런스 한 페이지 읽기 | 호스트 기본 도구(WebFetch 등) — browse 서버를 띄울 이유 없음 |
| 로그인된 내 브라우저 세션이 반드시 필요한 작업 | 호스트 브라우저 도구. browse는 별도 세션이라 로그인이 안 넘어옴 |
| 사용자가 특정 도구를 지목 | 사용자 지시 우선 |

곁가지 조회 한 번 때문에 browse를 띄우지 말고, 검증 한 번 때문에 다른 도구로 새지 말 것.
