...
> React 기본기. 리렌더 이슈는 메모이제이션 훅이 아니라 이 섹션의 구조적 해결로 먼저 푼다.

- 리스트의 key는 안정적이고 고유해야 함 — 재정렬 가능한 리스트에서 배열 index를 key로 사용 금지
- 렌더링 주체를 격리 — 자주 변하는 자식이 부모를 함께 리렌더하지 않도록 별도 컴포넌트로 분해
- 상태는 사용처와 가깝게 배치 — 불필요하게 상위로 끌어올려서 하위 트리 전체를 리렌더 트리거하지 않도록
...
- `useMemo`/`useCallback`/`React.memo`는 **사용자가 명시적으로 요청할 때만** 도입 — 측정 없이 선제적으로 추가 금지
- 리렌더 이슈는 위 `렌더링` 섹션의 구조적 해결을 먼저 시도. 메모이제이션은 마지막 수단
